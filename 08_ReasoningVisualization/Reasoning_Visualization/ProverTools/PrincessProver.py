...
class PrincessProver(IProverTool):
    ...
    def _exec_and_get_output(self, program_file_name, program_file_directory) -> str:
        pipe = subprocess.Popen([
            "princess",
            "-timeout=" + str(self.timeout_in_seconds * 1000),
            "-inputFormat=tptp",
            "+quiet",
            "-logo",
            program_file_name
    ...
    def _parse_output(self, output_text):
        ...
        if szs_status_line.startswith(self.SZS_prefix + "Timeout"):
            return ProverResult(ProverResultType.TIMEOUT, output_text)
        elif szs_status_line.startswith(self.SZS_prefix + "Theorem"):
